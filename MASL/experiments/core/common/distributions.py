...
class SoftCategoricalPd(Pd):
    def __init__(self, logits):
        #print('init self.logits', logits)
        self.logits = logits
    def flatparam(self):
        #print('flat self.logits', self.logits)
        return self.logits
    def mode(self):
        return U.softmax(self.logits, axis=-1)
    def logp(self, x):# softmax(logits) 和 labels 之间的交叉熵
        #print("-----")
    ...
    def log_prob(self):
        ...
        ea0 = tf.exp(a0)
        z0 = U.sum(ea0, axis=1, keepdims=True)
        p0 = ea0 / z0#a0 - tf.log(z0)
        return a0 - tf.log(z0)
    def kl(self, other):#KL(p(x)||q(x))
        a0 = self.logits - U.max(self.logits, axis=1, keepdims=True)
        a1 = other.logits - U.max(other.logits, axis=1, keepdims=True)#q(x)
        ea0 = tf.exp(a0)
        ea1 = tf.exp(a1)
...
def make_pdtype(ac_space):
    ...
    if isinstance(ac_space, spaces.Box):
        ...
        return SoftCategoricalPdType(ac_space[0])
        #raise NotImplementedError
