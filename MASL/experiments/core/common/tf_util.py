...
def load_state(fname, saver=None):
    ...
    #model_file = saver.recover_last_checkpoints(fname)
    #saver.restore(get_session(), model_file)
    saver.restore(get_session(), fname)
    return saver


def save_state(fname, saver=None,step=0):
    """Save all the variables in the current session to the location <fname>"""
    os.makedirs(os.path.dirname(fname), exist_ok=True)
    if saver is None:
        saver = tf.train.Saver()
    #saver.save(get_session(), fname,global_step=step)
    ...
    def __call__(self, *args, **kwargs):
        ...
        for inpt, value in zip(self.inputs, args):
            self._feed_input(feed_dict, inpt, value)
                ...
                #raise RuntimeError("input Nan detected")
